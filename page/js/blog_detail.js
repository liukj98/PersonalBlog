...
const blogDetail = new Vue({
    ...
    created(){
        let blogId = location.search.split("?")[1].split("=")[1]
        axios({
            url: "/query_blog_by_id?blogId=" + blogId,
            method: "get",
        }).then((res) => {
            ...
            this.c_time = changeTime(blogData.c_time);
        })
    }
})
...
const sendComment = new Vue({
    ...
    methods: {
        ...
        sendComment(){
            ...
            let blogId = location.search.split("?")[1].split("=")[1];// 博客id 表示你要对哪个博客进行评论
